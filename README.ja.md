...
| | |
|---|---|
| 最大 splat 数 | **4,508,391**（FDF-2026-08-22）。117 万を 3 シーン同時も問題なし |
| 描画時間 | **9.0 ms**（317 万 splats の drjohnson、RTX 3060、ドローン目線・画角 120°） |
| コリジョン | **ぶつかれる** — FDF-2026-08-22 から焼いた 2,197,134 三角形。物理は 400 Hz なので、150 km/h では厚さ 10 cm 未満の壁をすり抜ける |
...
**公開しているのは 2 本で、companion がそれを入れる。** どちらもこのプロジェクトのために
撮って **CC0** で出したもので、コリジョン付き。

| | splats | どこ |
|---|---|---|
| FDF-2026-08-22 | 4,508,391 | 船橋ドローンフィールド（千葉県船橋市の FPV 練習場） |
| JDL-2026-R5 | 2,521,003 | Japan Drone League 2026 Round 5 のレース会場（岡山県） |

**それ以外に開発中で飛んだものは再配布できないし、ここにも入っていない。**
学術データセットは**そもそもライセンスを表記していない**（それは「自由」ではなく全権利留保）、
INRIA 3DGS のライセンスは研究目的限定でその制限が派生物にも及び、残りは SuperSplat に
公開された第三者のキャプチャで、作者が選んだライセンス次第。リポジトリに入っている
splat データは `tools/make_test_ply.py` が生成する合成シーンだけ（軸・色・スケールの検証用）。

`.ply` は自分で用意して `<game>/vdgs/` に置ける。コリジョンは手元で焼く —
