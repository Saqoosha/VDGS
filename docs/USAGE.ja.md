...
**いちばん簡単なのは companion アプリ。** `VDGS.exe` を起動してタブ `01 setup` で
`Install mod` を押すと、DLL・焼き済みシェーダーバンドル・操作 UI が入る（**アプリが中に
持っている**ので、zip を探す必要はない）。配布中のトラックと導入済みのトラックはタブ
`02 tracks` に 1 本の表で並び、`Get` で導入できる。自分のキャプチャから新しいトラックを
作るならタブ `03 create your own` — 名前・紐付け・置き位置の調整までそこで完結する
（[TRACKS.ja.md](TRACKS.ja.md)）。`Fly` は `-force-d3d12` を必ず付けて起動する。
...
直さなくていい、軸の向きと鏡像だけ合わせたいなら、`.ply` を直置きしてタブ
`03 create your own` の③にある Up（6 方向）・Mirror・Turn（連続値のヨー）で合わせるほうが
...
Turn・Mirror は全部タブ `03 create your own` の③（合わせる）から動かせて、その場で
...
手で書いてもいいが、**companion のタブ `03 create your own` で新しいトラックを作ると
...
companion の `03 create your own` の②はまっさらな新しいトラックを作って紐付ける機能で、
**既存のトラックへの紐付け直しはまだそこからはできない。**

`placement.json` は**そのトラックに合わせた位置**なので、自分でコースを組むなら
タブ `03 create your own` の③で調整する（変更は自動保存される）。

## 5. 置き位置の調整

**Scale・Height・X・Z・Up・Turn・Mirror は全部、companion のタブ `03 create your own` の
③（合わせる）から動かす。** ゲームが動いている間だけ有効で、変更はその場で
`placement.json` に書かれる。飛びながら合わせるのが前提の画面なので、companion 自身の
窓がそのまま使える。

**別の画面から操作したいときだけ、プラグインが立てるブラウザ版を使う。** ゲームが
起動すると mod が **`http://<ホスト>:8777/`** で companion と同じアプリを配る——ただし
ブラウザには Tauri のフォルダ選択やダウンロードが無いので、タブ 01・02 は出ず
**③だけが動く**。companion はこの URL と QR をタブ 03 の③のすぐ横に出す。全画面の
...
**紐付けは自動。** タブ `03 create your own` の②でトラックを作った瞬間、そのキャプチャが
...
タブ `03 create your own` の③が使っているものと同じ。スクリプトから叩ける。
