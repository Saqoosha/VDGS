...
| 見た目 | particles 既定。survey とヘッダで切替 | 3DGS そのものの粒子場。測量票も残す |
| 言語 | 英語 | 現行 UI と同じ |
...
今の見た目は **particles**（暗い場に異方性のガウシアンが漂う）を既定にする。ヘッダで **survey**（測量野帳）と切り替えられる。どちらもライト／ダーク HUD の既製品ではない。CDN は使わない。
