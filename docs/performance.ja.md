...
## 3. LOD（Streamed SOG で実装済み）

**再訓練は要らなかった。** SuperSplat の Streamed SOG（`lod-meta.json`）は splat-transform が
段ごとに decimate した集合を空間の木の葉に分けて持っているので、それを全段まとめて常駐させ、
葉ごとに 1 段だけ描く。置き方と罠は CLAUDE.md の「Streamed SOG と LOD」、設計は
...
ベンチは https://superspl.at/scene/7a7bfaea（17,281,128 splats、5 段、葉 2,324）。
`VDGS_BENCH_INSIDE=1 VDGS_BENCH_CAM=1.4,-1.7,0,0 bash tools/bench-win.sh 7a7bfaea,7a7bfaea-l2`。

| 構成 | 常駐 | 描いた splat | 平均 |
|---|---|---|---|
| 空 | — | 0 | 2.6〜3.2 ms |
| 単段 level 2 の ply | 2.24M | 2.24M | 9.3 ms |
| ssog、LOD 既定 | 17.3M | 2.98M | **10.9 ms** |
| ssog、全葉 level 0（実質 LOD 無し） | 17.3M | 8.89M | 23.1 ms |

**LOD で 23.1 → 10.9 ms。** 常駐 17.3M のうち描くのは 2.98M。ここに至るまでに 2 つ入れた：

- **選ばれた splat だけを鍵バッファの先頭に詰め、距離パスとソートをその数だけ回す。**
  15.4 → 11.2 ms。詰め替えは選択が変わったフレームだけ（ソートは同じ集合を並べ替えるだけで、
  集合は変わらない）
- **段を「見かけの大きさ」で選ぶ**（次節）。描画数はむしろ減って 2.98M

**LOD を持たない ply が master より 0.85 ms（約 9%）遅い。未解明。** `-vdgsSortNth` で
切り分けるとソート側 0.5 ms・ビュー側 0.3 ms。潰した仮説 2 つ —— LOD バッファを束ねるのを
やめる（**描画ごと消える**。`LoadSplatData` が名前を出すだけでカーネルが「使う」扱いになり、
Unity がディスパッチを捨てる）、`SetKeyword` の回数を減らす（変化なし）。次に見るなら
1 パスずつ計る道具から。

**Mac エディタ（M1 Max）の数字は判断に使わない。** 同じ構成で LOD 既定が平均 27〜72 ms と
揺れ、全葉 level 0 は 241 ms まで崩れた（RTX 3060 は 23 ms）。ユニファイドメモリで 17.3M 常駐の
圧迫を受けていると見ている。
...
3. **VP8L のビット読みを 64 ビット窓に**（decode 2.8〜3.1 → 2.4〜2.7 s）。1 ビットずつ読み、記号ごとに 4 バイトを組み直していた
4. **読み込みを裏スレッド、アップロードを 1 フレーム 4 ms ずつ**。`byte[]` のまま `SetData` に渡す（`uint[]` への 738 MB の詰め替えを廃止。添字がバイト単位で効くことは Metal と D3D12 の両方で読み戻して確認）

| 機械 | 実行環境 | read | decode | pack | 合計 | メインスレッドの最悪フレーム |
|---|---|---|---|---|---|---|
| RTX 3060 機 | **ゲーム内**（`/api/load`） | 0.1〜0.2 s | 2.4〜2.6 s | 1.4 s | **4.1 s** | 約 120 ms（色テクスチャの 1 回） |

色テクスチャだけは分割できない（`Texture2D.SetPixelData` はミップ丸ごと）。

以前（同じ姫路 17.3M）：

| 機械 | 実行環境 | read | decode | pack | 合計 |
|---|---|---|---|---|---|
| RTX 3060 機 | **ゲーム内**（`/api/load`） | 0.2 s | 7.6 s | 8.3 s | **16.1 s** |
| M1 Max | エディタ batch（`SogCheck`、2026-09-24） | 0.2 s | 6.4〜6.9 s | 7.8〜8.6 s | 14.4〜15.7 s |
| RTX 3060 機 | エディタ Debug | 0.3 s | 6.8 s | 9.4 s | 16.6 s |
| M1 Max | Debug（既定） | 0.7 s | 33 s | 33 s | 67 s |
...
| 5 | LOD（Streamed SOG） | 遠景のみ劣化 | RTX 3060 で 23.1 → 10.9 ms（17.3M 常駐、2.98M 描画） | **実装済み**（常駐分のソートと読み込み時間が未解決） |
