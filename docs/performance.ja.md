...
**ただしこの順位はベンチのもので、ベンチはゲームではない。** 実機で飛んだことがあるのは
Float32（26.83 ms）と Cluster16k（17.30 ms）の 2 つだけで、**High は一度も飛んでいない**。
ここでの High 対 Cluster16k の差は 0.20 ms。しかもベンチは固定カメラ、ゲームは動くカメラで、
カリングは視点次第で 41〜97% と振れる。**0.20 ms より遥かに大きいレバー**。
**忠実さは決着している**（Float32 比 0.09/255、画素で測った）。**High と Cluster16k の
速度の順位は決着していない。** 実機がひっくり返したら、実機が正しい。
