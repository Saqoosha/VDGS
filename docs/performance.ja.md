...
選択は距離帯 + 予算 + ヒステリシス（`LodSelector`）。非選択の run は `CSCalcDistances` で最大
ソートキーに駐車し、`CSCalcViewData` の先頭で抜ける — **常駐の 87% を占める splat ごとの固定
コストを、選ばれていない splat は払わない。**
...
**ゲーム内で 16 秒止まる。** Windows ではエディタとほぼ同じで、Release コードでも縮まない。
VP8L 単体は .NET 8 でチャンク 1 つ（748² × 7 枚）0.6 秒なので、Mono との差が大きい。
Mac でスレッドプールの最小数を上げても 61 秒で変わらなかった（スレッド不足ではない）。pack の
疑いは `PackRotation` の splat ごとの `new float[4]`（PlyLoader から無改変で移したコード）。
...
| 5 | LOD（Streamed SOG） | 遠景のみ劣化 | RTX 3060 で 23.1 → 15.4 ms（17.3M 常駐、2.97M 描画） | **実装済み**（常駐分のソートと読み込み時間が未解決） |
