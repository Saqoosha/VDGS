...
**トラックは行ごと消せる**（各行の `REMOVE`）。消えるのは `user11.db` の行と
`bindings.json` の項目、**そしていまは紐付いたキャプチャも**。以前はキャプチャを残していた
——GB 単位で、トラックの削除は小さなクリックだから、というのが理由だった。だが実装後
最初に試した本人がトラックを 2 本消し、どちらのキャプチャも導入済みとして
理由も無く残っているのを見つけた——**その理屈は、何が残るか本人に分かって初めて成り立つ**
もので、分からなかった。確認ダイアログはキャプチャの名前を挙げ、カタログから再取得できる
か、元の `.ply` が無いともう戻せないかを言う。**公式サーバー由来のトラックは削除しない**
（ボタンは `UNBIND` になる）。作者のものであって、こちらが人の機械から消していいもの
ではない——消えるのはバインディングだけで、キャプチャは「installed, on no track」に
残り、そこの remove で消せる。DB は削除前にコピーする
（**ラップタイムは再取得不能**）。
...
同じ `get` — 新しい cut を `vdgs/.<installAs>.new/` に展開してから rename で入れ替える
（途中で失敗しても旧フォルダは無傷、新 cut に無いファイルは残らない）。`placement.json` と
紐付けは温存。**`revision` を知らない
