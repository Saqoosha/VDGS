...
**0.5 なら 22% 速いが、絵が変わる。** 無損失は 15%。

余白がここまで要るのは、**1 つの余白でシーン中で最も大きい splat を守らねばならない**から。
キャプチャには巨大で拡散した splat が数個混じっていて、その数個のために全体が広い余白を
払っている。

**次の一手：余白を splat ごとに出す。** スケールから世界空間の半径を求め、クリップ空間に
投影すれば、大きい splat だけが広い余白を得る。大多数の小さい splat は 0.5 相当で足り、
無損失のまま 22% 側に近づけるはず。`CSCalcDistances` は今は位置しか読んでいないので、
スケールの読み出しを足す必要がある（chunk 相対のときの復号も含めて）。
