...
導入と起動は [USAGE.ja.md](USAGE.ja.md)、この上にコースを組んで配るなら
[TRACKS.ja.md](TRACKS.ja.md)。このファイルはキャプチャ側のパイプライン。
...
**自分で学習するなら 3DGUT で焼かない。** `--with_ut` で学習した splat を VDGS や
SuperSplat（EWA 族）で描くと **PSNR 24.5 → 12.4 で完全に崩壊する**。劣化ではなく崩壊で、
しかも**目視では捕まえられない** — ビューアに出た「それらしい絵」がすでに崩壊後の姿だった。
ckpt はレンダラーとセットで初めて意味を持つ。**出口が EWA なら学習も EWA。**
