...
### これはパースの時間。体感する停止は約 3 倍ある

`PlyBench` が測るのは header・read・decode・sort まで。**`GraphicsBuffer.SetData` の前で
止まる**が、アップロードはタダではない。だから 0.97 秒を「ロード時間」として出すのは
画面で起きることを過小に言っている。ゲーム内の実測（`vdgs-perf.log`、スポーンした
フレームが `worst_ms` に出る）：

```
                splats     SH    ファイル    実機の停止
nelson-lod2  2,171,895   なし   121.6 MB      2.95 s
calico-lod3  2,401,279   なし   134.5 MB      3.10 s   （2 回とも同値）
nelson-full  8,759,558   なし   490.5 MB     11.97 s
utlida-lod1  2,001,694   3 次   472.4 MB      6.77 s
textilni     2,320,155   3 次   547.6 MB      8.00 s
utlida-full  4,003,388   3 次   944.8 MB     13.0 / 13.2 / 14.0 s
```

ここから 2 つ出る：

- **レートはバイトではなく splat ごとで、しかも異様に安定している**。SH 無しで
  1.34 µs/splat、3 次で 3.39 µs/splat。上のどのシーンも自分のグループの 4% 以内
- **SH があるとロードが 2.5 倍。** バイトあたりで見ると SH 付きのほうが**速い**
  （約 70 MB/s 対 42 MB/s）。splat ごとの仕事がより多くのデータに薄まるため

つまり 400 万 splats の SH 付きキャプチャは**出るまで 13 秒**かかる。2026-09-24 から解析は
裏スレッドで走るのでゲームは止まらないが、色テクスチャのアップロードで 1 回（姫路 17.3M で約 120 ms）
引っかかる。「飛ぶ前に表示させておく」はそのため。

**ただしこれはコールドの数字で、ウォームだと約 4 倍速い。** 同じファイル、同じビルド、
同一ブート内での 2 回目：
...
「初めて触る人が何を体験するか」を答えるならコールドを引く。それが正直な既定。ウォームを
引くのは、そのブートで既に読まれていると分かっているときだけ — たとえばベンチのループは
2 周目以降ウォーム側を測るので、放っておくと**自分に都合のいい数字を出す**。
