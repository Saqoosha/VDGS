#!/usr/bin/env bash
# Run a local script inside WSL on the Windows box, where vdb_tool and the decimator live.
#
...
# vdb_print but not vdb_tool. WSL has it from apt (libopenvdb-tools).
#
...
SCRIPT="${1:?usage: tools/wsl.sh <script> [root]}"
USER_ARG=""
[ "${2:-}" = "root" ] && USER_ARG="-u root"
DISTRO="${VDGS_WSL_DISTRO:-Ubuntu-24.04}"
B64="$(base64 < "$SCRIPT" | tr -d '\n')"
TAG="$(basename "$SCRIPT" .sh)-$$"
ssh -o ConnectTimeout=15 "$HOST" \
  "wsl -d $DISTRO $USER_ARG -e bash -c 'f=\$(mktemp /tmp/wslrun-$TAG-XXXX.sh); echo $B64 | base64 -d > \$f; bash \$f; rc=\$?; rm -f \$f; exit \$rc'" \
