...
if [ -n "$CAM" ]; then
  ...
  scp -o BatchMode=yes -q "$CAM" "$HOST:$REMOTE_ROOT/vdgs-stage/evalcam.json" 2>&1 | quiet
  ssh -o BatchMode=yes "$HOST" \
    "Copy-Item (Join-Path (Join-Path $REMOTE_ROOT_PS 'vdgs-stage') 'evalcam.json') (Join-Path \$env:USERPROFILE 'Downloads\\Velocidrone Windows Launcher\\app\\vdgs\\evalcam.json') -Force" 2>&1 | quiet
fi

scp -o BatchMode=yes -q "$ROOT/tools/evalshot-win.ps1" "$HOST:$REMOTE_ROOT/vdgs-stage/evalshot-win.ps1" 2>&1 | quiet
ssh -o BatchMode=yes "$HOST" \
  "\$s = Join-Path (Join-Path $REMOTE_ROOT_PS 'vdgs-stage') 'evalshot-win.ps1'; \$d = Join-Path $REMOTE_ROOT_PS 'evalshot-win.ps1'; Copy-Item \$s \$d -Force; powershell -ExecutionPolicy Bypass -File \$d" 2>&1 | quiet

scp -o BatchMode=yes -q "$HOST:$REMOTE_ROOT/vdgs-shot.png" "$OUT" 2>&1 | quiet
echo "-> $OUT"
