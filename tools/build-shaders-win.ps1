...
# Everything this repo puts on the box lives under one directory (tools/_remote.sh).
$vdgs    = Join-Path $homeDir 'VDGS'
$tgz     = Join-Path $vdgs 'vdgs-bundler.tgz'
$root    = $vdgs
$project = Join-Path $vdgs 'VDGSBundler'
$outDir  = Join-Path $vdgs 'vdgs-bundles'
...
$log1 = Join-Path $vdgs 'vdgs-shaderbuild-1.log'
...
$log2 = Join-Path $vdgs 'vdgs-shaderbuild-2.log'
