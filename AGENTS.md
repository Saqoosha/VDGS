...
**companion は LAN アドレスと QR を Tweak 画面の先頭に出す**。存在を
伝える場所がどこにも無かった問題への対処——以前は `local · lan :8777` という 1 行が
UI の中にしかなく、たどり着いた人にしか見えなかった。全画面のゲームから alt-tab するのが
最悪の操作なので、**合わせようとした瞬間にだけ**別画面という選択肢を出す。
