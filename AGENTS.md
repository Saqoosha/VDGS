...
読み込みは裏スレッドで走り、画面の左上に「VDGS loading <名前> - decoding 12/36」のような 1 行が
出る。アップロードは 1 フレーム 4 ms ずつに分けてあり、残る山は色テクスチャの 1 回（17.3M で
約 120 ms）。飛行中に切り替えると小さく引っかかるので、**飛ぶ前に表示させておく**。
...
**`PlyBench` の数字は `GraphicsBuffer.SetData` の前で止まる。** ゲーム内で表示までにかかる時間は
217 万で 2.95 秒、400 万＋SH で 13〜14 秒（2026-09-24 より前の測定。当時はこの間ゲームが止まった）。**引くのはコールドの値**（同一ブート 2 回目は
...
### `placement.json` の姿勢フィールド

`position` / `rotation` / `scale` に加えて 3 つ増えた——`up`（`+x -x +y -y +z -z` のどれが
空を向くか）、`turn`（`up` を軸にした回転、度）、`mirrorY`（読み込み時に Y を鏡映するか）。
適用順は固定：`.ply / bins を読む → mirrorY を適用 → rotation（up/turn 由来）を適用 →
position/scale を適用`。

**鏡映と回転は別物で、どちらも他方の代わりにならない。** 3DGS は右手系 Y-down、
Unity は左手系 Y-up なので、届いたキャプチャはそのままだと鏡像になる。鏡映は行列式 -1 の
変換で、回転（行列式 +1）をいくら足しても同じ結果にはならない。だから `mirrorY` は
`up`/`turn` とは独立に、**データを読む段階**で適用する。既定は現状維持——`.ply` は
`true`、変換済みディレクトリは `false`。

**`mirrorY` は変換済みキャプチャには効かない。** 鏡映はパース中に起きる処理で、
`SplatData.Load`（変換済みディレクトリ）はパックされたバッファをそのまま読むだけだから。
`SetOrientation` は変換済みキャプチャへの mirror 変更要求を、ログに残して黙って無視する
（持続させても見た目が変わらないまま despawn/respawn だけ払うことになるため）。

**旗は splat とコリジョンの両方が読む。** `SplatCollision` は元々拡張子だけで鏡映を決めて
いた（`.ply` なら必ず鏡映）ので、そこと `PlyLoader` が別々に判断すると壁だけ鏡像になる。
`SplatScene.MirrorFor(Placement)` が両方に同じ値を渡すことで揃えている——**壊れても
エラーは 1 行も出ない**ので、この関数を経由せずに `mirrorY` を読む場所を増やさないこと。

**`turn` が 90° 刻みではなく連続値である理由**：3DGS は光を SH に焼き込んでいて、
ゲーム側の太陽と向きが食い違うとドローンの機体だけ別方向から照らされる。太陽は
90° の倍数にはいないので、刻みのボタンでは合わない。キャプチャ自身は無照明で
描かれるため影響を受けず、ずれるのは常時画面にいる機体だけ。

**背景の黒い箱は、キャプチャが直立しているときしか付かない。** `SplatBackdrop` は
床をローカル空間でクランプしているので、回転した状態のまま付けると天地が回って
クランプした面が壁になり、キャプチャの手前に黒い面が出る。`up` が `+y` から動くと
...
- **読み込みは裏スレッド、アップロードはフレームに分ける。** RTX 3060 機のゲーム内で 17.3M が
  4.1 秒で読み終わり、アップロードが 1 秒前後続く。その間ゲームは動き、左上に段階が出る。最悪のフレームは
  色テクスチャの約 120 ms。以前は 16 秒の完全停止。内訳は docs/performance.ja.md「読み込み時間」
...
## 操作は companion 内蔵の UI（ゲーム内キーではない）

`http://<host>:8777/` でプラグインが立てる HTTP サーバーは、**companion と同じ 1 画面の
コンポーネント木**を配っている（`web/index.html` と companion 本体が読む
`companion.html` は同じコンポーネントを読む）。別の UI ではなく、同じアプリを 2 つの
入口から出しているだけ。

**ホストが無いと導入の帯・トラックの表・Fly は出さない。** どれもフォルダ選択と
ダウンロードが要り（Tauri のダイアログ経由）、それの無いプレーンなブラウザでは原理的に
動けない。`bridge.ts` の `hosted = !!window.__TAURI__` が false のとき、これらは
disabled のボタンとして残るのではなく**そもそも描かれない**。
**Tweak 画面だけが動く** — 見た目はいまの `Control` に近いが、
同じコンポーネントから出るので**片方だけ直る事故は起きない**。

見た目は暗い場にガウシアンが漂う。カードは使わない。静的ファイルは
`<game>/vdgs/ui/`。無いときは `WebUi.cs` が短い案内だけ出す。

**companion は LAN アドレスと QR を出さない（2026-09-16 に外した）。** Tweak 画面の先頭に
出していたが、ブラウザ版はスマホ向けの調整がまだで、案内だけ先に出すと誘導先が使い物に
ならない。モバイルのレイアウトを整えたら戻す。`lanUrl` は host の state に残っている。
...
- **`TrackBindings.Load()` はコンストラクタでしか呼ばれていなかった。** プラグインは
  起動時に `bindings.json` を 1 回読んでメモリの写しで動くため、ゲーム稼働中に companion が
  ファイルを直接書いても気づかず、次にプラグインが `Save()` した瞬間に**古い内容で
  上書きしていた**。エラーは 1 行も出ない。症状は「companion から Unbind しても効かない」
  としてしか見えない。**直した** — `PollTrack` が既に 1 秒ごとに回っているので、そこで
  `bindings.json` の書き込み時刻と長さを見て、変わっていれば `Load()` し直す
  （`ReloadIfChanged`）。同じ 1 秒の中で 2 回書かれると時刻だけでは区別できないので、
  長さも一緒に比べる。自分の `Save()` 直後は自分が書いた時刻を控えて無視するので、
  誤って自分の書き込みを読み直すことはない。**ただし読み直すだけでは Unbind は
  直っていなかった。** 外した先が現在のトラックのままだと、再読み込み後に呼ぶ `ApplyTrackBinding`
  自身の「紐付けの無いトラックには触らない」規則（`/api/load` で手動スポーンした画を
  無関係なポーリングで消さないための規則）がここでも働いて、何も起きない。companion は
  成功と言い、プラグインもログに再読み込みを残すのに、画面のキャプチャだけ残る——症状は
  直る前とそっくり同じだった。**これも直した**（2026-09-05）——`PollTrack` の再適用分岐が
  `m_Bindings.Has` を先に見て、無くなっていれば `ApplyTrackBinding` を呼ばず、
  `/api/unbind`（`Unbind`）と同じ despawn をここでも行う。ファイルが「消した」と言っている
  以上、これは推測ではなく指示に従っているだけ。手で `bindings.json` を編集した場合も、
  外すのと差し替えるのと、どちらも 1 秒で反映される
...
変わったら GS を入れ替える（`PollTrack`）。同じポーリングで `bindings.json` の
変更も見ているので、companion がゲーム稼働中に書き換えても 1 秒以内に反映される
（「罠」の `TrackBindings.Load()` を参照）。
...
**画面は 1 枚**（導入の帯 / トラックの表 / Fly）。ゲームを探して mod を入れる、配布中と
導入済みのトラックを 1 本の表で扱う、自分の `.ply` を持ち込んでトラックを作って飛びながら
合わせる — この 3 段が同じ窓に並んでいる。タブをやめたのは 2026-09-16 の通し確認で、
2 つのタブが「トラック」という 1 つの概念を割っていたと分かったから——ファイルを取る
ボタンが片方にあり、欲しいファイルはもう片方にあり、Create の成功直後は「nothing
installed yet」に見えた（成功で一覧が空になる設計だった）。ログは導入側にしか無く、
失敗するボタンは表と作成側にあった。いまは表にキャプチャ列ごと畳んである。

**操作はゲームの状態で半分ずつ入れ替わる。** `Add track` / `Get` / `Remove` はファイルの
置き換えと `user11.db` への書き込みが要るので、ゲームが動いていないときしか押せない
（`add_track` が既に `launch::is_running()` で弾いている規則がそのまま乗る）。`Tweak` は
相手がプラグインの HTTP サーバーなので、逆にゲームが動いていないと存在しない（画面の
トラックがその行と一致しているときだけ生きる）。`Unbind` はどちらでも動く。押せない側は
理由を名乗る — 画面そのものが「準備する → 飛ぶ → 合わせる」という順番になっている。

`Add track` でトラックを作る瞬間に紐付けも同じ処理で書くので、**「組んでから名前を変えて
絵が消える」という順番の罠がここでは起きない**（詳しくは
[docs/TRACKS.ja.md](docs/TRACKS.ja.md)）。
...
- **`companion/tests/` にしか無かった検査**。とくに 2 本 ——
  **ペイロードの資産を数えて各エントリ 1 本ずつであることを確かめるもの**
  （23 本入って 5 リリース気づかれなかった事故の見張り。いまは `rm -rf`
  してから組むので**溜まらない**が、**数えてはいない**）と、
  **BepInEx の pin を実際に取りに行くもの**（URL・バイト数・sha256 は
  **コードを誰も触らずに腐る**。いまはローカルの fixture しか見ていない）
...
そして `std::process::exit` は**フラッシュしない**ので、明示的に流す。
**どちらも「終了コードだけ正しくて出力が空」という形で出る**
（端末は行バッファなので気づけない）。
...
viewer の `mark` で人がランドマークを打ち `marks_apply.sh` で解く（1 周 1 時間、印の残差 32 → 7 px）。
