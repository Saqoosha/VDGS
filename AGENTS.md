...
`http://<host>:8777/` でプラグインが立てる HTTP サーバーは、**companion と同じ 3 タブの
コンポーネント木**を配っている（`web/index.html` と companion 本体が読む
`companion.html` は同じコンポーネントを読む）。別の UI ではなく、同じアプリを 2 つの
入口から出しているだけ。

**ホストが無いとタブ 01・02 は出さない。** どちらもフォルダ選択とダウンロードが要り
（Tauri のダイアログ経由）、それの無いプレーンなブラウザでは原理的に動けない。
`bridge.ts` の `hosted = !!window.__TAURI__` が false のとき、この 2 つは disabled の
ボタンとして残るのではなく**そもそも描かれない**。
**タブ 03 の「合わせる」半分だけが動く** — 見た目はいまの `Control` に近いが、
同じコンポーネントから出るので**片方だけ直る事故は起きない**。

見た目は暗い場にガウシアンが漂う。カードは使わない。静的ファイルは
`<game>/vdgs/ui/`。無いときは `WebUi.cs` が短い案内だけ出す。

**companion は LAN アドレスと QR をタブ 03 の合わせる操作のすぐ横に出す**。存在を
...
**画面は 3 タブ**（`01 setup` / `02 tracks` / `03 create your own`）。ゲームを探して mod を
入れる、配布中と導入済みのトラックを 1 本の表で扱う、自分の `.ply` を持ち込んでトラックを
作って飛びながら合わせる — この 3 段がそのままタブになっている。前は同じ名詞（キャプチャ・
トラック）が companion と `:8777` の UI に分かれて 3 箇所ずつ出ていて、紐付けを直せる画面を
誰も知らなかった。いまは 1 本の表（タブ 02）にキャプチャ列ごと畳んである。

**タブ 03 は半分ずつゲームの状態で入れ替わる。** ①「入れる」と②「作る」はファイルの
置き換えと `user11.db` への書き込みが要るので、ゲームが動いていないときしか押せない
（`add_track` が既に `launch::is_running()` で弾いている規則がそのまま乗る）。③「合わせる」
は相手がプラグインの HTTP サーバーなので、逆にゲームが動いていないと存在しない。押せない
側は理由を名乗る — タブそのものが「準備する → 飛ぶ → 合わせる」という順番になっている。

② でトラックを作る瞬間に紐付けも同じ処理で書くので、**「組んでから名前を変えて絵が消える」
という順番の罠がここでは起きない**（詳しくは [docs/TRACKS.ja.md](docs/TRACKS.ja.md)）。
