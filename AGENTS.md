...
`<game>/vdgs/autospawn`（空ファイル）が無い場合は自動表示そのものが無効。**mod を入れる
経路がこれを作る**（`INSTALL MOD` と mod zip の両方）。2026-09-01 まで作る者がいなくて、
**companion で新規導入した機械は全部これで無言のまま何も映らなかった** — キャプチャも
トラックも紐付けも正しいのに。既にある場合は触らない。消すことが自動表示を切る手段なので、
入れ直しが勝手に戻すと選択を奪う。
