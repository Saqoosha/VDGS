...
**dev ビルド**。カタログの `minModVersion` は日付なので、両方が日付形のときだけ比較する
（`Catalog.Entry.ModShortfall`）。**0.1.0.0 は「古い」ではなく「比較不能」として通す** —
古い扱いにすると、いま動いている全インストールが唯一の公開キャプチャを取れなくなる。
実際には問題なく描ける組み合わせなので。つまり **dev ビルドは決してゲートされない**。

この経路は 2026-09-01 まで**繋がっていなかった**。`minModVersion` は読まれるだけで
どこにも比較が無く、公開中の FDF に `2026.08.25` が入ったまま何もしていなかった
（[#4](https://github.com/Saqoosha/VDGS/issues/4)）。**版を刻む前にゲートだけ繋ぐと全員が
ブロックされる**ので、順番を逆にしないこと。
